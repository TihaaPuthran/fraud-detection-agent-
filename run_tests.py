...
def test_normal_transaction():
    print("\n" + "=" * 60)
    print("TEST 1: NORMAL TRANSACTION (Expected: LOW or MEDIUM risk)")
    print("=" * 60)
    ...
    assert result["risk_level"] in ["LOW", "MEDIUM"], (
        f"Expected LOW or MEDIUM, got {result['risk_level']}"
    )
    assert result["fraud_probability"] < 0.7, (
        f"Expected probability < 0.7, got {result['fraud_probability']}"
    )

    print("\n[PASS] Test 1 passed: Normal transaction correctly identified")
    return True
...
def test_feature_consistency():
    ...
    from app.ml.feature_builder import build_features, FEATURE_NAMES
    from app.ml.data_generator import generate_structured_dataset
